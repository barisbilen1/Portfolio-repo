---
  title: <center> <h3> Fall 2020 </h3> <h2> IE 451 Applied Data Analysis </h2> <h3> taught by Savaş Dayanık </h3>  <h2> Homework 6 </h2>  <h3> done by </h3> <h3> <span style="color:red">*Ali Barış Bilen* </span> </h3>  <h4> <span style="color:red">*21602902* </span> </h4></center>
  pagetitle: IE 451 Homework 6 Fall 2020 
  date: <center> Do not return. For self-study. We will post the solutions by 6 January 2021, Wednesday. </center>
  output: 
...
@. [Grades.csv](Grades.csv) reports the grades of students on various tasks in a statistics course. Try several models to explain the variation in `Final` exam grades bymeans of the other assessment grades. Select the best of all your models. Justify your selection. I expect you to explore data with graphics, check non-linearities and interactions with effect plots, regression diagnostics, do variable selection, report how good your model fits.

@. In [Grades.csv](Grades.csv), create a new variable `FinalAboveAverage` defined as "Yes" if `Final` is greater than the `Final` average and "No" otherwise. Remove `Final` grades from the data. Try several models to explain the variation in `FinalAboveAverage` bymeans of the other assessment grades. Select the best of all your models. Justify your selection. I expect you to explore data with graphics, check non-linearities and interactions with effect plots, regression diagnostics, do variable selection, report how good your model fits.



```{r}
df2<- df%>%mutate(FinalAboveAverage=as.factor(ifelse(Final>=mean(Final),"Yes","No"))) %>% dplyr::select(-Final)
```

```{r}
glm.0<-glm(FinalAboveAverage~Midterm1+Midterm2+Homework+Project,data= df2, family="binomial")
...
```{r}
library(effects)
allEffects(glm.0, partial.residuals = TRUE) %>% plot()
```
```{r}
glm.1<-glm(FinalAboveAverage~Midterm1+Midterm2,data= df2, family="binomial")
